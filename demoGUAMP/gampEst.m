...
% Check for the presence of custom error function #1
if ~isempty(opt.errFcn1)
    errFcn1 = opt.errFcn1;
    customErr1 = true;
else
    customErr1 = false;
...
%   pass = (~adaptStep) || (step <= stepMin) || isempty(valMin) || (val >= valMin);
    ...
    if (pass)
        ...
        if (it>1) && (stop==false)
            if all(sum(abs(xhatPrevFinal-xhatFinal).^2,1)./sum(abs(xhatFinal).^2,1) < tol^2)
                stop = true;
            elseif customStop==1 
        ...
        % Error functions
        if customErr1
            err1(it,:) = errFcn1(xhatFinal,AxhatFinal,rhatFinal,phatFinal);
        end
        if customErr2
            err2(it,:) = errFcn2(xhatFinal,AxhatFinal,rhatFinal,phatFinal);
        end
        ...
        if opt.B==1
            ...
            EstimOut2bit = opt.EstimOut2bit;
            [zhat,zvar] = EstimOut2bit(phat,pvarRobust);
        end
...
if saveHist
    ...
    if (it < nit)
        ...
        estHist.xhat(:,it+1:nit) = repmat(estHist.xhat(:,nitTrim(end)),1,nit-it);
...
if (legacyOut)
    ...
    estFin.nit = it;
    estFin.err1 = err1(1:it,:);
    estFin.err2 = err2(1:it,:);
    out1 = estFin;
    out2 = opt;
    if (saveHist)
        out3 = estHist;
